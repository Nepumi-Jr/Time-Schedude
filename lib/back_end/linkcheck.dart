...
Link_Check(String input) {
  ...
  while ((matcher.hasMatch(input)) == true) {                     //if there have link in text
    app = Linkify(                                                //check for link can launch
    onOpen: (link) async {
    ...
    i = i + 1;
    input = input.replaceAll(matcher.stringMatch(input).toString(), "");      //เป็น text ที่ตัด link อันแรกแล้ว
    myList[i] = matcher.stringMatch(input).toString();                        //link จะถูกเก็บในarray mylist
