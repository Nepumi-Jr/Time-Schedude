...
class _EditPageState extends State<EditPage> {
  ...
  initialTimeToEdit(List<TimeSub> time) {
    print(time);
    for (var i = 0; i < time.length; i++) {
      value[time[i].dayOfWeek - 1] = !value[time[i].dayOfWeek - 1];
      time_start[time[i].dayOfWeek - 1] =
          TimeOfDay(hour: time[i].hourStart, minute: time[i].minuteStart);
      time_end[time[i].dayOfWeek - 1] =
          TimeOfDay(hour: time[i].hourEnd, minute: time[i].minuteEnd);
      print(time[i].dayOfWeek - 1);
      print(time[i].hourStart);
      print(time[i].minuteStart);
      print(time[i].hourEnd);
      print(time[i].minuteEnd);
    }
  }
  ...
  void initState() {
    ...
    _subject_name = TextEditingController(text: widget.subject_name);
    _link = TextEditingController(text: widget.link);
    _place_name = TextEditingController(text: widget.place);
    allTimeLearn = TimeTable.callTimeByName(widget.subject_name);
    ...
    initialTimeToEdit(allTimeLearn);
  }
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () {
                ...
                Navigator.of(context).pop();
                Navigator.of(context).pop();
                TimeTable.removeSubject(widget.subject_name);
                getInformationToClassTimeTable();
                /* AwesomeDialog(
                  context: context,
                        ...
                        child: Row(
                          ...
                          children: [
                            InkWell(
                              onTap: () {
                                print("******* Hey,Remove ******* ");
                                print(widget.subject_name + "has removed.");
                                TimeTable.removeSubject(widget.subject_name);
                                Navigator.of(context).pop();
                              },
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    gradient: LinearGradient(
                                        colors: [
                                          color.AppColor.Gradient7,
                                          color.AppColor.Gradient7
                                              .withOpacity(1),
                                          color.AppColor.Gradient8
                                              .withOpacity(1),
                                          color.AppColor.Gradient8,
                                          //add more colors for gradient
                                        ],
